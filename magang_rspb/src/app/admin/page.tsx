...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchPolis = async () => {
      try {
        ...
        if (!res.ok) return;
        const data = await res.json();
        const normalized: Poli[] = (data || []).map((p: any) => ({
          id: p.id,
          name: p.name,
        }));
        setPolis(normalized);
  ...
  const stats = useMemo(() => {
    ...
    const menunggu = registrations.filter(
      (r) => (r.status || "").toLowerCase() === "menunggu"
    ).length;
    const dipanggil = registrations.filter(
      (r) => (r.status || "").toLowerCase() === "dipanggil"
    ).length;
    const selesai = registrations.filter(
  ...
  const sortedByQueue = useMemo(() => {
    return [...filtered].sort((a, b) => {
      ...
      if (aHasQueue && bHasQueue)
        return (a.queue_number as number) - (b.queue_number as number);
  ...
  const callPatient = async (patient: Registration) => {
    try {
      ...
      await updateRegistration(patient.id, {
        status: "dipanggil",
        queue_number: queue,
      });
    } catch (err: any) {
  ...
  const callNextPatient = async () => {
    const candidates = sortedByQueue.filter(
      (r) => (r.status || "").toLowerCase() === "menunggu"
    );
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-gray-600">
                  Kelola pendaftaran pasien dari tabel registrations
                </p>
              </div>
              <div className="flex flex-col gap-3 md:flex-row md:items-center">
                ...
                <button
                  ...
                  disabled={
                    loading || callingId !== null || filtered.length === 0
                  }
          ...
          <div className="p-6 border-b border-gray-200 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Data langsung dari tabel registrations
              </p>
            </div>
            {loading && (
